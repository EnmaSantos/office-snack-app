 ...
 */
function seedSnacks() {
    const count = (0, index_1.get)('SELECT COUNT(*) as cnt FROM Snacks');
    ...
    for (const snack of defaultSnacks) {
        (0, index_1.run)('INSERT INTO Snacks (Name, Price, Stock, IsAvailable) VALUES (?, ?, ?, 1)', [snack.name, snack.price, snack.stock]);
        const snackId = (0, index_1.lastInsertRowId)();
        (0, index_1.run)('INSERT INTO SnackBatches (SnackId, QuantityPurchased, QuantityRemaining, UnitCost, PurchaseDate) VALUES (?, ?, ?, ?, ?)', [snackId, snack.stock, snack.stock, snack.unitCost, now]);
    }
